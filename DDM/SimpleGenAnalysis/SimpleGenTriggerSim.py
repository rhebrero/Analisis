...
h_lxy       = createSimple1DPlot("h_lxy", "lxy"        , 600,  0., 600., samples)
...
#    if index !=0: continue
    ...
    for indexSubsample, ksubsample in enumerate(ksample):## To debug 
        ...
        events = Events(ksample)    
        if indexSubsample == 1: continue #for debugging?
    ...
    #       print ("BEAMSPOT ", hltOnlineBeamSpot.x0(), hltOnlineBeamSpot.y0(), hltOnlineBeamSpot.z0())
            ...
            if len(triggerList) > 0:
                for i in range(triggerBits.size()):
                    if triggerBits.accept(i) and triggered == False:
...
makeSimple1DPlot(h_dphi,     'h_dphi',     samples, '', 'dphi',       '', 'h_dphi',     outFolder, logy=True, norm=False)
